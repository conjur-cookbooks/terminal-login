...
describe "terminal-login::configure" do
  ...
  context "from conjur.conf and netrc" do
    let(:conjur_conf) {
      YAML.dump({
...
machine https://conjur.example.com/api/authn
  login host/the-netrc-host
  password the-netrc-host-api-key
NETRC
    }
    ...
    it "creates /etc/nslcd.conf" do
      subject.should create_template("/etc/nslcd.conf").with(
        variables: {
          account: "test",
          host_id: "the-netrc-host", 
          host_api_key: "the-netrc-host-api-key",
          gid: "nslcd",
          uri: "ldaps://conjur.example.com", 
          cacertfile: "/etc/conjur.pem"
        }
      )
    end
  end
